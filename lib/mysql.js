...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    if (s.pool) {
        ...
        connectionSettings.waitForConnections = s.waitForConnections || true;
        ...
        schema.client = mysql.createConnection(s.url ? s.url : connectionSettings);
    }
    schema.client.on('error', function(err) {
        ...
        initializeConnection(schema.client, function(err){
            if (!err){
                console.log('Re-connection Success');
            }
        });
    ...
    initializeConnection(schema.client, callback);

    function initializeConnection(connection, callback) {
        connection.query('USE `' + connection.config.database + '`', function (err) {
            if (err) {
                if (err.message.match(/(^|: )unknown database/i)) {
                    ...
                    schema.client.query(q, function (error) {
                        if (!error) {
                            schema.client.query('USE ' + connection.config.database, callback);
                        } else {
                            throw error;
                        }
                    });
